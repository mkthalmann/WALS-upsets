...
Set up some options and load the required packages for the current project. Chiefly among them, since they're the driving forces of the visualizations to come, are *eulerr* [@larsson2020eulerr], *venneuler* [@wilkinson2011venneuler], and *UpSetR* [@gehlenborg2019UpSetR]. 
...
The data I will work with in the course of this project is from @dryer2013wals (World Atlas of Language Structures, https://wals.info), and I will download the underlying data sets directly from the Github page where they're hosted (https://github.com/cldf-datasets/wals). To reduce computation times, I will subset it right from the very beginning to only focus on same parameters to do with constituent ordering. Note: while we want to check intersections within word order, we will disregard those languages with dual word order patterns (code `81B`) to simplifiy result interpretation.
...
Before proceeding, we need to make sure that there's no missing data. Additionally, we need to know what kinds of columms we're working with. Table \ref{tab:check} shows that we have four different parameters, at least one of which has 7 different possible values. 
...
Because of the way that the different plotting methods we'll use later handle data, we need to transform it into wide format. In Table \ref{tab:widen}, you can see the output of this transformation: each parameter is now instantiated in its own column.
...
The values might be nicer if they were human-readable, so we'll add them in the next steps. The descriptions are contained in `codes.csv`. The relevant part is displayed in Table \ref{tab:codes}. 
...
Below is the code to rename the parameters and add the descriptions. As the descriptions are very long at times, I opted for manual entry as opposed to simple replacements with the values from Table \ref{tab:codes}. For the outcome, see Table \ref{tab:add-info}.
...
Before finally turning our attention towards the actual visualizations, we will perform one (largely optional) step: adding some information about the `r length(d$language_id)` languages whose parameters we're visualizing. This information won't actually be displayed in the set intersetion plots, but I think it makes the final data set more complete, so I'll add it anyway. Plus, it allows us to visualize our language sample on a world map, as you'll see in the next section.

As before, the language information is contained in yet another WALS data set, `languages.csv`. The columns of interest, namely those we will be adding to our reshaped data, are shown in Table \ref{tab:languages}.
...
After combining the language information with our data, we are left with Table \ref{tab:lang-comb}.
...
Using the *sf* package [@pebesma2018simplefeatures], let's look at what kinds of languages we have data on (and to actually make use of the coordinate information). The output is shown in Figure \ref{fig:world}.
...
Let's start with the first Venn/Euler diagram^[See https://en.wikipedia.org/wiki/Euler_diagram for a description of Euler diagrams, as well as differentiation from Venn diagrams.] [using the *venneuler* package, @wilkinson2011venneuler]. Note that in Figure \ref{fig:venn} there does not seem to be a way of using ellipses instead of circles for the shape of the sets, which leads to awkward layout design and loss of intersection information in the present case.
...
The second Venn/Euler diagram, Figure \ref{fig:venn-2}, was created with the *eulerr* package [@larsson2020eulerr]. Here, it is possible to use an ellipse as the basic set shape, which allows a more information rich display (note the OVS and OSV overlaps that could not be shown previously).

You may have noticed that I opted for generating a legend as opposed to adding the set names directly within the ellipses. This way done primarily so that as many intersections as possible can be displayed. Of course, this comes at the price of readability. Different from Figure \ref{fig:venn}, the current plot cannot be interpreted as quickly, simply because a legend lookup is required (and, potentially, because my color choice may not be the best one out there). 
...
The alternative, with the set names noted within the sets themselves, is shown in Figure \ref{fig:venn-3}. As before, while the larger sets are unaffected by this choice, the results for the smaller ones, OVS and OSV, are quite different. 
...
Finally, to overcome some of the problems we encountered with Venn/Euler diagrams, we have the up set visualization in Figure \ref{fig:upset-extended}, created with the *UpSetR* package [@gehlenborg2019UpSetR; see also @lex2014sets]. Here, because the basic type of plot is a bar diagram, all intersections can be displayed without needing to be concerned about the geometric shape of the sets as either ellipses or circles. This also allows for the inclusion of two other parameters: ordering of demonstratives with respect to the noun as well as numeral positioning. 
...
For comparison, Figure \ref{fig:upset-standard} is the up set version of the Venn/Euler diagrams (i.e., exculding demonstrative and numeral position). 
